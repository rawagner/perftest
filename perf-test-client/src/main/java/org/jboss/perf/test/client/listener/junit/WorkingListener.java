...
import org.jboss.perf.test.client.monitor.MonitorCore;
...
import org.jboss.perf.test.objects.objects.LastTestSuiteData;
import org.jboss.perf.test.objects.objects.TestSuiteData;

public class WorkingListener extends RunListener {
	private static final Logger logger = Logger.getLogger(WorkingListener.class);
	...
	public WorkingListener(BaseConfig config) {
		logger.debug("TESTING WITH CONFIGURATION:\n\nPROJECT:\t" + config.getProject()
		    ...
		    + "\nHW PLATFORM:\t{}" + config.getPlatform()
		    + "\nREP_URL:\t " + config.getRepUrl() + "\n");
